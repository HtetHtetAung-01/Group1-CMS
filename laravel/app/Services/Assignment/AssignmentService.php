...
class AssignmentService implements AssignmentServiceInterface
{
    ...
    public function addStudentAssignment($student_id, $course_id, $assignment_id, $filename)
    {
        ...
        return $this->assignmentDao->addStudentAssignment(
                $student_id,
                $course_id,
                $assignment_id,
                $inputFileName
    ...
    {
        return $this->studentAssignmentDao->getAssignmentStatusByStudent(
                $student_id,
                $assignment_id
            );
    }
    ...
    public function isCompletedAssignment($student_id, $course_id)
    {
        ...
        foreach ($assignment_details as $assignment) {
            $status = $this->studentAssignmentDao->getAssignmentStatusByStudent(
                    $student_id,
                    $assignment->id
                );
    ...
    {
        $start = [];
        $assignmentList = $this->assignmentDao->getAllAssignmentByCourse($course_id);

        foreach ($assignmentList as $key => $values) {
            $start[$key] = $this->assignmentDao->isStarted(
                    $student_id,
