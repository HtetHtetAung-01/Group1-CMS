...
@section('assets')
    <link rel="stylesheet" href="{{ asset('css/teacher_assignment.css') }}">
@endsection
        ...
        $("form.cmt-msg").submit(function(e) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.success) {
                        const messageRow = '<p class="cmt-text"><b>' + commentor + ':&nbsp;</b>' +
                            commentBody + '</p>';
                                                    ...
                                                    <tbody>
                                                        @foreach ($course->assignmentList as $item)
                                                            <tr class="tr-record">
                                                                ...
                                                                <td class="file-path">
                                                                    <a
                                                                        href="{{ route('teacher.assignment.download', ['id' => Auth::user()->id, 'assignment_id' => $item->id]) }}">
                                                                        {{ basename($item->file_path) }}
                                                                    </a>
                                                                </td>
                                                                <td id="student_grade{{ $item->id }}">
                                                                    {{ $item->grade != null ? $item->grade . '%' : '' }}</td>
                                                                <td>
                                                                    <button class="open-button"
                                                                        onclick="openForm({{ Auth::user()->id }},{{ $item->id }})">Submit
                                                                        Grade</button>
                                                                </td>
                                                            </tr>
                                                            <tr>
                                                                <td colspan="4">
                                                                    ...
                                                                    <!-- /.accd -->
                                                                    <form class="cmt-msg"
                                                                        action="{{ route('teacher.assignment.comment', ['id' => Auth::user()->id, 'assignment_id' => $item->id]) }}"
                                                                        method="post">
                                                                        @csrf
