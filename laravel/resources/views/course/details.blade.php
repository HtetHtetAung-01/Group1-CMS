        ...
        <div class="course-details">
            <div class="course-content">
                <div class="title-btn">
                    ...
                    <button data-modal="modal-enroll"
                        class="btn-show-modal default-enroll-btn {{ $isEnrolled ? 'start-btn' : 'disabled-btn' }}">{{ $isEnrolled ? 'Get Started' : 'Enrolled' }}</button>
                            ...
                            <div class="modalContent">
                                ...
                                <div class="mdl-inner">
                                    ...
                                    <div class="mdl-btns">
                                        ...
                                        <a href="{{ route('course-enroll', ['id' => Auth::user()->id, 'course_id' => $courseDetails[0]->course_id]) }}"
                                            class="confirm-btn">
                                            Confirm
                                        </a>
                                    </div><!-- /.mdl-btns -->
                    ...
                    @foreach ($courseDetails as $key => $value)
                        <div class="accd-li course">
                            <dt class="accd-dt d-flex">
                                <div class="d-flex">
                                    @if ($isEnrolled == false)
                                        @if ($assignmentStatus != null && $key < count($assignmentStatus) && $assignmentStatus[$key] == 'completed')
                                            <img src="/img/completed_icon.png" alt="progress-icon">
                                        @else
                            ...
                            <dd class="accd-dd">
                                <div class="accd-content">
                                    @if ($started[$key] == false)
                                        <button
                                            class="btn-show-modal start-assign-btn {{ $isEnrolled ? 'disabled-btn' : 'start-assignment' }}"
                                            onclick="openForm('{{ route('assignment-start', ['id' => Auth::user()->id, 'course_id' => $courseDetails[0]->course_id, 'assignment_id' => $courseDetails[$key]->id]) }}')">Start</button>
                                    @endif
                                    ...
                                    <div class="resources-blk">
                                        ...
                                        @else
                                            <h3 class="resources-lbl {{ $isEnrolled ? 'disabled-lbl' : '' }}">Resources
                                            </h3>
                                        @endif
                                        <div class="resources d-flex">
                                            ...
                                            @endif
                                            @if ($started[$key] == false)
                                                <a href="{{ route('assignment-resource', ['id' => Auth::user()->id, 'course_id' => $courseDetails[0]->course_id, 'assignment_id' => $courseDetails[$key]->id]) }}"
                                                    class="default-download-btn disabled-btn">
                                                    Download File
                                                </a>
                                            @else
                                                <a href="{{ route('assignment-resource', ['id' => Auth::user()->id, 'course_id' => $courseDetails[0]->course_id, 'assignment_id' => $courseDetails[$key]->id]) }}"
                                                    class="default-download-btn {{ $isEnrolled ? 'disabled-btn' : 'download-btn' }}">
                                                    Download File
                                                </a>
                                            @endif
                                        </div><!-- /.resources -->
                                    ...
                                    <div class="homework-blk">
                                        ...
                                        @endif
                                        <form
                                            action="{{ route('assignment-submission', ['id' => Auth::user()->id, 'course_id' => $courseDetails[0]->course_id, 'assignment_id' => $courseDetails[$key]->id]) }}"
                                            ...
                                            <div class="homework d-flex">
                                                ...
                                                {{ csrf_field() }}
                                                @if ($started[$key] == false)
                                                    <input type="file" name="inputFile" id="input-file-name"
                                                        class="default-file-input disabled-file-input" disabled />
                                                @else
                                                    <input type="file" name="inputFile" id="input-file-name"
                                                        class="default-file-input {{ $isEnrolled ? 'disabled-file-input' : 'file-input' }}"
                                                        {{ $isEnrolled ? 'disabled' : '' }} />
                                                @endif
                                            </div>
                                            <div class="d-flex">
                                                @if ($started[$key] == false)
                                                    <a href="javascript:void(0)"
                                                        onclick="document.getElementById('input-file-name').value='';"
                                                        class="default-clear-btn disabled-clear-btn">
                                                        Clear
                                                    </a>
                                                @else
                                                    <a href="javascript:void(0)"
                                                        onclick="document.getElementById('input-file-name').value='';"
                                                        class="default-clear-btn {{ $isEnrolled ? 'disabled-clear-btn' : 'clear-btn' }}">
                                                        Clear
                                                    </a>
                                                @endif
                                                @if ($started[$key] == false)
                                                ...
                                                @else
                                                    <button type="submit"
                                                        class="default-submit-btn {{ $isEnrolled ? 'disabled-btn' : 'submit-btn' }}">
                                                        Submit
                                                    </button>
                                                @endif
                                            </div>
...
@else <div class="msg-box-empty">
        <p>No Assignment Yet. Come back later.</p>
    </div>
    @endif
